...
float pwm_out = 0.6;//initial pwm_out set to 0.6
float vb = 0;
...
void setup() {
 ...
 void loop() {
    ...
    if (Boost_mode){
      if (CL_mode) { //Closed Loop Boost
          ...
          //pwm_modulate(open_loop); // and send it out
            ...
            //pwm_out = saturation(pwm_out, 0.99, 0.33); //duty_cycle saturation
  ...
  if(int_count==1000){//slow loop
    //MPPT (boost)
          sampling();
          Power_now = vb * iL; //assume current ripple small? Do we need to get an average value for inductor/output current? How? Saving measurements to SD cards then take out?
