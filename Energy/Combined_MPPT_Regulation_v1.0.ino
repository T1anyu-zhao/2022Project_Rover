...
void setup() {
  ...
  pinMode(14, OUTPUT); // ****for MPPT part //****pin ~D11 //****if can?
  
  pinMode(2, INPUT_PULLUP); // Pin 2 is the input from the CL/OL switch //****for Regulator //****pin D0
  pinMode(1, INPUT_PULLUP); //****for MPPT part //****pin D1 //****if can?
    
  pinMode(6, OUTPUT); // This is the PWM Pin //****for Regulator //****pin ~D3
  pinMode(9, OUTPUT); //****for MPPT part //****pin D6
  
  //LEDs on pin 7 and 8
  pinMode(7, OUTPUT); //error led //****for Regulator //****pin D4
  pinMode(5, OUTPUT); //****for MPPT part //****pin D2
  
  pinMode(8, OUTPUT); //some digital output //****for Regulator //****pin ~D5
  pinMode(12, OUTPUT); //****for MPPT part //****pin ~D9

  pinMode(10, OUTPUT); //relay //****for Regulator //****pin D7
...
void loop() {
  if (loop_trigger == 1){ // FAST LOOP (1kHZ) (for changing duty cycle)
      ...
      sensorValue4 = analogRead(A5); //****MPPT part
      
      //sensorValue2 = analogRead(A2); //sample Vref (desired output voltage)
      //sensorValue5 = analogRead(A6); //****MPPT part
      ...
      if (voltageOutput_m < 4.50) { //Checking for Error states (input voltage lower than battery minimum charging voltage)
          digitalWrite(5, true); //turn on the red LED
          pwm_out_m = 0; // no PWM
      }
      ...
      analogWrite(9, (int)(255 - pwm_out_m * 255)); // write it out (inverting for the Buck here)
  ...
  if (int_count == 1000) { // SLOW LOOP (1Hz) (for MPPT and relay operation)
    ...
    //****MPPT
    input_switch_m = digitalRead(1); //get the OL/CL switch status
