...
{ 
  ...
  if (angle_45<=360)
  {
    ...
    while(!turn &&convertTodegree(current_angle)<360 )
    {
      ...
      Serial.println("Desire angle = " + String(angle_45));
      ...
      delay(250);
      turn = rotate_constant(angle_45, dy_mm, dx_mm, &current_angle);
    }
    if (turn){
    angle_45 = angle_45 + 45;
    delay(100);
    }
  }
  return orientation;
...
void generate_coordinates(){
  ...
  if(check_block(angle_o, d_o_v)){
    ...
    float a1 = current_x + magnitude * sin(desire_angle);
    float a2 = current_y + magnitude * cos(desire_angle);

    
    //add second point to finish divesion
    float b1 = current_x + base * sin(current_angle);
    float b2 = current_y + base * cos(current_angle);
