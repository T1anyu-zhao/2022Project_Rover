...
int length_c = 2000, width_c = 2400;
int increment = 600;
int p = 0;
float desire_ang;
...
bool distance_control(int desired_x, int desired_y, int desire_an)
{
    ...
    bool forward = (((current_y + error_distance * cos(desire_an) < desired_y + 21) && (current_y + error_distance * cos(desire_an) > desired_y - 21))) ? true : false;
    int speed = 0;
    cout << "desire angle  is: " << desire_an << endl;
    ...
    if (error_distance > 20 && !forward) // move backward
    {
        ...
        { //
            speed = fabs(control_distance) * 0.04;
            if (speed < 15)
            {
                speed = 15;
            }
...
void go_straight(float angle_o, float d_o_v){
    ...
    cout << "a(" << a1 << ", " << a2 << ")" << endl;
      ...
      {//////////////////////////////////////////////////////////
        ...
        delay(250);
        arrive = distance_control(a1, a2,desire_angle);
    }
  ...
  delay(250);
    ...
    if(set_line_end){
      ...
      for(int  o = 0; o < 1; o++){
        ...
        coordinate_list.push_back(length_c-700-500);
  ...
  cout << " ------- destination(x,y) is: (" << destination_x << ", " << destination_y << ")" << endl;
    ...
    {  cout << "j = " << j << endl;
      ...
      if (reachDestination)
        ...
        bool turn_done;
        // rotate
        desire_ang = calculate_desried_angle(destination_x, destination_y);
        turn_done = angle_control(convertTodegree(desire_ang), dy_mm, dx_mm, &current_angle);
        ...
        while (turn_done && !reachDestination)
        {
           ...
           // }
            //else{
                    reachDestination = distance_control(destination_x, destination_y, desire_ang);
                //}
